...
current_time = time.strftime('_%m-%d-%H-%M-%S', time.localtime(time.time()))
current_dir = '{}/train-val{}'.format(out_dir, current_time)
counter = 1
while os.path.exists(current_dir):
    current_dir = '{}/train-val{}-{}'.format(out_dir, current_time, counter)
    counter += 1
...
def train(args, train_loader, model, criterion, optimizer, epoch, epoch_size):
    ...
    for i, (input_var, target) in enumerate(train_loader):
        ...
        # Convert prec1 and prec5 to scalar values
        prec1_scalar = prec1.item()
        prec5_scalar = prec5.item()
        losses.update(loss.item(), input_var.size(0))
        top1.update(prec1_scalar, input_var.size(0))
        top5.update(prec5_scalar, input_var.size(0))
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                ...
                epoch+10, i, len(train_loader), batch_time=batch_time,
...
def validate(args, val_loader, model, criterion):
    ...
    for i, (input_var, target) in enumerate(val_loader):
        ...
        prec1, prec5 = accuracy(output.data, target, topk=(1, 5))
        losses.update(loss.item(), input_var.size(0))
        top1.update(prec1[0], input_var.item())
        top5.update(prec5[0], input_var.item())
...
def accuracy(output, target, topk=(1,)):
    ...
    res = []
    for k in topk:
        correct_k = correct[:k].reshape(-1).float().sum(0)  # 修改这行
